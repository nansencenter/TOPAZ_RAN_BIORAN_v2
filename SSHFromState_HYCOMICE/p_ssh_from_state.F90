...
!      KAL -- March 2007, calculates SLA as well as SSH, requires meanssh  file to be    present
    ...
    program p_ssh_from_state
       ...
       character(len=80):: filename, filenameSSH,depthfile, fbase, matfile, ncfname
      ...
      real, allocatable, dimension(:,:) ::   &
         ...
         depths,modlon, modlat, meanssh, sla, fice, hice, uice, vice
      real, allocatable, dimension(:, :, :) :: dp3d, saln3d, temp3d
      ...
      integer :: sla_id, temp_id, salt_id, dp_id, p_id 
      integer :: fice_id, hice_id, uice_id, vice_id
...
!
      ...
      ncfname = trim(fbase) // '.nc'
      print *, 'dumping fields to NetCDF file "', trim(ncfname), '"'

      call nfw_create(ncfname, nf_write, ncid)
      call nfw_def_dim(ncfname, ncid, 'i', idm, dimids(1))
      call nfw_def_dim(ncfname, ncid, 'j', jdm, dimids(2))
      call nfw_def_dim(ncfname, ncid, 'k', kdm, dimids(3))
      
      call nfw_def_var(ncfname, ncid, 'lon', nf_double, 2, dimids, lon_id)
      call nfw_def_var(ncfname, ncid, 'lat', nf_double, 2, dimids, lat_id)
      call nfw_def_var(ncfname, ncid, 'depth', nf_double, 2, dimids, depth_id)
      call nfw_def_var(ncfname, ncid, 'ssh', nf_double, 2, dimids, ssh_id)
      call nfw_def_var(ncfname, ncid, 'meanssh', nf_double, 2, dimids, meanssh_id)
      call nfw_def_var(ncfname, ncid, 'sla', nf_double, 2, dimids, sla_id)
      call nfw_def_var(ncfname, ncid, 'temp', nf_float, 3, dimids, temp_id)
      call nfw_def_var(ncfname, ncid, 'salt', nf_float, 3, dimids, salt_id)
      call nfw_def_var(ncfname, ncid, 'dp', nf_float, 3, dimids, dp_id)
      call nfw_def_var(ncfname, ncid, 'p', nf_float, 3, dimids, p_id)
      call nfw_def_var(ncfname, ncid, 'fice', nf_float, 3, dimids, fice_id)
      call nfw_def_var(ncfname, ncid, 'hice', nf_float, 3, dimids, hice_id)
      
      call nfw_enddef(ncfname, ncid)

      call nfw_put_var_double(ncfname, ncid, lon_id, modlon)
      call nfw_put_var_double(ncfname, ncid, lat_id, modlat)
      call nfw_put_var_double(ncfname, ncid, ssh_id, ssh)
      call nfw_put_var_double(ncfname, ncid, meanssh_id, meanssh)
      call nfw_put_var_double(ncfname, ncid, sla_id, sla)
      call nfw_put_var_double(ncfname, ncid, hice_id, hice)
      call nfw_put_var_double(ncfname, ncid, fice_id, fice)
      call nfw_put_var_double(ncfname, ncid, temp_id, temp3d)
      call nfw_put_var_double(ncfname, ncid, salt_id, saln3d)
      call nfw_put_var_double(ncfname, ncid, dp_id, dp3d)
      call nfw_put_var_double(ncfname, ncid, p_id, p(:, :, 2 : kdm + 1))
      call nfw_close(ncfname, ncid)
