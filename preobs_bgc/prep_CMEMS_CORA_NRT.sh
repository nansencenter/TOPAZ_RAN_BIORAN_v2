...
gdate=20250301     # Gregorian date [YYYYMMDD]
CNFG=TP2           # short name of hycom configuration [TP5|TP2]
DSRC=CMEMS_CORA_NRT # short name of dataset
...
if $isload; then
    ...
    echo "#-------------------------"
    echo "# cmems_cora_loader.py ${gdate} NRT \"$types_string\" $isload"
    echo "#-------------------------"
    python cmems_cora_loader.py ${gdate} NRT "$types_string" $isload
else
